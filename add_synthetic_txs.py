...
The script aims to facilitate the creation of an OCC dataset for tasks like object detection and tracking.
"""
...
bit_duration = 0.13333  # Duration of one bit in seconds
frame_duration = 2  # Duration of one frame in seconds
points = []
...
def modulate_image(frame, center, radius, bit, energy):
    ...
    if bit == 1:       
        ...
        mask = np.array(energy * create_mask(radius, radius/np.sqrt(2*np.log(radius))))[..., np.newaxis].repeat(3, axis=2)
...
def create_YoLo_file(transmitters, radius, width, height, output_file):
    ...
    Args:
        ...
        radius(np.array): Written radius for each transmitter.
...
def store_pixels(transmitters, fps, output_file):
    """
    Store pixel positions of selected transmitters and non-transmitters.
...
def main():
    ...
    Generates and stores random data frames for each transmitter and modifies  
    the images to represent those data frames in the specified positions and sizes.
    """
    # Loop through directories
    for i in range(640, 647):    #229, 232
        input_path = f'data/{i}' 
        try: 
            print(i)
            ...
            clip = VideoFileClip(f'data/{i}/video.mp4').subclip(0, 4)
            clip.preview()
            clip.close()
            ...
            while (frame_number < total_video_frames) and (frame_number < 30*fps): # max 30 sec of video
                ...
                if frame_number % number_frame_duration == 0:
                    # Generate the bit sequence for each luminous region
